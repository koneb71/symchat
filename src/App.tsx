...
function App() {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (!isVision) {
          processImageOCR(imageData)
            ...
            toast({
              title: 'Image Pasted',
              description: isVision ? 'Image ready to send' : 'Processing OCR...',
            })
          }
        }
      }
      ...
      if (documents.length > 0 && !ragEnabled) {
        ...
        if (previousState === false) {
          toast({
            ...
            description: `Found ${documents.length} document${documents.length !== 1 ? 's' : ''} in your library`,
          })
        }
  ...
  const loadConversations = async () => {
    try {
      ...
      setConversations(convs.filter(c => c.id).map(c => ({ ...c, id: c.id! })))
  ...
  const handleImageUploadInline = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast({
      title: 'Image Added',
      description: isVision ? `${files.length} image(s) ready to send` : 'Processing OCR...',
    })
  }

  const handleFileUploadInline = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const files = event.target.files
    if (!files || files.length === 0) return

    const { isSupportedFileType, getFileTypeName, parseDocument } = await import('./lib/document-parser')
  ...
  const handleSendMessage = async (content: string) => {
    ...
    if (generationOptions.use_custom_system_prompt && generationOptions.system_prompt) {
    ...
    if (uploadedFiles.length > 0) {
      const processedFiles = uploadedFiles.filter(f => f.extractedText && !f.isProcessing && !f.error)
    ...
    if (deepResearchEnabled) {
      try {
        ...
        const { performDeepResearch, formatResearchContext } = await import('./lib/deep-research')
        
        const research = await performDeepResearch(content, (progress) => {
          // Could show progress toast here
          console.log(`Research progress: ${progress.currentStep}/${progress.totalSteps}`)
        })
        ...
        if (research.status === 'completed' && research.finalReport) {
          ...
          toast({
            ...
            description: `Gathered information from ${research.steps.filter(s => s.status === 'completed').length} research angles`,
          })
        }
      } catch (error) {
      ...
      try {
        ...
        if (searchResult.shouldSearch && searchResult.results) {
          const autoContext = formatAutoSearchContext(content, searchResult.results)
          systemContext += '\n\n' + autoContext
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: 'Failed to generate response. Make sure Ollama is running.',
        variant: 'destructive',
      })
    } finally {
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
              ...
              {generationOptions.use_custom_system_prompt && (
                <>
                  <span>•</span>
                  <span className="text-blue-500">Custom Prompt</span>
                </>
              )}
            </div>
          </div>
            ...
            {messages.length === 0 && !currentResponse ? (
              <div className="flex flex-col items-center justify-center h-full text-center p-8">
                ...
                <p className="text-muted-foreground max-w-md">
                  Start a conversation with your AI assistant. Ask questions, get help with
                  coding, or just chat about anything!
                </p>
              </div>
            ) : (
              <>
            ...
            <div className="border-t bg-background p-4">
              ...
              autoSearchEnabled={autoSearchEnabled}
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  {uploadedFiles.length > 0 && (
                    <span>
                      {(ragEnabled || autoSearchEnabled || deepResearchEnabled) && <span>•</span>}
                      {' '}{uploadedFiles.length} file{uploadedFiles.length !== 1 ? 's' : ''} attached
                    </span>
                  )}
                  {uploadedImages.length > 0 && (
                    <span>
                      {(ragEnabled || autoSearchEnabled || deepResearchEnabled || uploadedFiles.length > 0) && <span>•</span>}
